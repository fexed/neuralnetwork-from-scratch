            ...
            for M in units:
                for E in learning_rates:
                    for B in batch_sizes:
                        ...
                        net.add(FullyConnectedLayer(input_size, M, tanh, tanh_prime, init_f))
                        for i in range(N):  # N -hidden- layers, plus input and output layers
                            net.add(FullyConnectedLayer(M, M, tanh, tanh_prime, init_f))
                        net.add(FullyConnectedLayer(M, output_size, tanh, tanh_prime, init_f))
