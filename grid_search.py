...
def grid_search(input_size, output_size, X, y,
    ...
    n_combinations = len(layers)*len(units)*len(learning_rates)*len(init_functions)*len(momentums)*len(regularizators)*len(batch_sizes)*len(dropouts)*folds
                                    ...
                                    for init_f in init_functions:
                                        for B in batch_sizes:
                                            ...
                                            while (kfold.hasNext()):
                                                ...
                                                history, val_history, epochs_done = net.training_loop(xtr, ytr, X_validation=xvl, Y_validation=yvl, epochs=epochs, learning_rate=E, batch_size=B, verbose=verbose, early_stopping=early_stopping)
                                                final_losses_mean += val_history[-1]
                                                epochs_done_mean += epochs_done
                                                tested += 1
